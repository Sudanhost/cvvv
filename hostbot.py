...
# --- Configuration ---
TOKEN = '7952169581:AAH-5Hk-3pKEmTg7qhBe_SV8WA7kKgCj2x8' # Replace with your actual token
OWNER_ID = 8170404189 # Replace with your Owner ID
ADMIN_ID = 8170404189 # Replace with your Admin ID (can be same as Owner)
YOUR_USERNAME = '@V2ray_hack' # Replace with your Telegram username (without the @)
UPDATE_CHANNEL = 'https://t.me/Hostpython1' # Replace with your update channel link
...
COMMAND_BUTTONS_LAYOUT_USER_SPEC = [
    ["📢 Updates Channel", "⚡ Bot Speed"],
    ["📤 Upload File", "📂 Check Files"],
    ["📊 Statistics", "📦 Install Package"], # Statistics button kept for users, logic will restrict if not admin
    ["📞 Contact Owner"]
]
ADMIN_COMMAND_BUTTONS_LAYOUT_USER_SPEC = [
    ...
    ["📦 Install Package", "👑 Admin Panel"],  # أضفنا الزر هنا
    ["📞 Contact Owner"]
]
...
def create_main_menu_inline(user_id):
    ...
    if user_id in admin_ids:
        admin_buttons = [
            ...
            types.InlineKeyboardButton('📦 Install Package', callback_data='install_package')  # أضفنا الزر هنا
        ]
        ...
        markup.add(admin_buttons[6])  # Install Package
...
def _logic_install_package(message):

        
    
    msg = bot.reply_to(message, "📦 Enter package name to install (e.g., `requests` or `numpy`):\n/cancel to abort.")
    bot.register_next_step_handler(msg, process_package_installation)

def process_package_installation(message):
    
    if message.text and message.text.lower() == '/cancel':
        bot.reply_to(message, "Package installation cancelled.")
        return
    
    package_name = message.text.strip()
    if not package_name:
        bot.reply_to(message, "⚠️ Please enter a valid package name.")
        msg = bot.send_message(message.chat.id, "📦 Enter package name or /cancel:")
        bot.register_next_step_handler(msg, process_package_installation)
        return
    ...
    try:
        ...
        if result.returncode == 0:
            success_msg = f"✅ Package `{package_name}` installed successfully!\n\nOutput:\n```\n{result.stdout}\n```"
...
BUTTON_TEXT_TO_LOGIC = {
    ...
    "📦 Install Package": _logic_install_package,  # أضفنا الزر هنا
...
@bot.message_handler(commands=['installpackage'])
def command_install_package(message): 
    _logic_install_package(message)
...
def handle_callbacks(call):
    ...
    try:
        ...
        elif data == 'install_package': admin_required_callback(call, install_package_callback)  # أضف هذا السطر هنا
        else:
...
def install_package_callback(call):
    """معالج زر تثبيت الحزمة"""
    bot.answer_callback_query(call.id)
    _logic_install_package(call.message)     
